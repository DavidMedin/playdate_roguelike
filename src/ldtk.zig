// Author : David Medin
// Note : LDtk, Json, Zig, and Playdate are all causing me immense pain in this file.
...
const Enum = struct { externalRelPath: ?[]u8, iconTilesetUid: ?i32, tags: [][]u8, uid: i32, values: []EnumValue };

const GridValue = struct { identifier: ?[]u8, tile: ?TilesetRect, value: i32 };
...
const TilesetRect = struct { x: i32, y: i32, h: i32, w: i32, tilesetUid: i32 };
const Data = struct { data: []u8, tileId: i32 };
const EnumTags = struct { enumValueId: []u8, tileIds: []i32 };
...
const FieldInstance = struct { __identifier: []u8, __tile: ?TilesetRect, __type: []u8, defUid: i32 }; // __value: json.Value (we don't like Value)
...
pub fn test_json(playdate: *pdapi.PlaydateAPI, allocator: std.mem.Allocator) !void {
    ...
    if (bytes_read != stats.size) {
        std.log.err("Mismatched bytes I guess!\n", .{});
        return;
    }
